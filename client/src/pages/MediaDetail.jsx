...
import { Box, Button, Chip, Divider, Stack, Typography, Grid, Card, CardMedia, CardContent } from "@mui/material";
...
const MediaDetail = () => {
  ...
  useEffect(() => {
    const fetchSimilarMovies = async () => {
      try {
        const { response, err } = await mediaApi.getSimilar({ mediaType, mediaId });
        if (err) throw err;
        setSimilarMovies(response.data.results.slice(0, 4)); // Get top 4 similar movies
      } catch (error) {
  ...
  const handleBookTickets = () => {
    ...
    navigate(`/booking/${mediaId}`);
  };

  const handleCompare = (movieId) => {
    navigate(routesGen.movieComparison.path.replace(':movie1Id', media.id).replace(':movie2Id', movieId));
  };
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <Button
                        variant="outlined"
                        size="small"
                        onClick={() => handleCompare(movie.id)}
                        sx={{ mt: 1 }}
                      >
                        Compare
                      </Button>
